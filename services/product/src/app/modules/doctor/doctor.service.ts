...
const getPatientsAvailableSchedule = async (
  ...
  const isBookedBoolean =
    ...
    console.log({isBookedBoolean});

  const scheduleConditions: Prisma.ScheduleWhereInput[] = [];
  ...
  const whereConditions: Prisma.DoctorSchedulesWhereInput = {
    ...
    schedule:
      scheduleConditions.length > 0 ? { AND: scheduleConditions } : undefined,
  };

  const patientSchedule = await prisma.doctorSchedules.findMany({
    where: whereConditions,
    skip,
    take: limit,
