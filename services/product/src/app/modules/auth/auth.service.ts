...
const setPassword = async (user: any, newPassword: string) => {
  ...
  const hasGoogle = isExistingUser.authProviders.some(
    (auth: any) => auth.provider === "google",
  );

  if (isExistingUser.password && hasGoogle) {
  ...
  const providerData = {
    ...
    providerId: isExistingUser.email,
  };
